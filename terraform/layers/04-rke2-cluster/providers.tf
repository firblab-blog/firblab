# =============================================================================
# Layer 04: RKE2 Cluster - Provider Configuration
# =============================================================================
# Deploys a DISA STIG-hardened RKE2 Kubernetes cluster on Proxmox VMs.
# 3 server nodes + 2 agent nodes on the Services VLAN (20).
#
...
locals {
  ...
  # Derive Proxmox host IP from the API URL for guest-agent SSH key injection.
  # The URL in Vault is e.g. "https://10.0.10.42:8006" — extract the host part.
  proxmox_ssh_host = local.proxmox_api_url != "" ? regex("https?://([^:/]+)", local.proxmox_api_url)[0] : ""
}
